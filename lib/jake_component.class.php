...
class JakeComponent
{
	...
	/**
	 * Restore Joomla's DB connection after CakePHP application execution.
	 * 
	 * @since 1.0
	 * @var bool
	 */
	...
	function getParameter($parameter, $default = null)
	{
		...
		{
			$result = mosGetParam( $_REQUEST, $parameter, $default );
		}
	 ...
	 * @param bool $atLeast	true to check if Joomla's version is same or greater than, false otherwise
	 * 
	 * @return bool	
	 * 
	 * @access protected
	 * @since 1.0
	 */
	function compareJoomlaVersion($version, $atLeast = true)
	{
		$operator = 'ge';
		...
		return version_compare($this->joomlaVersion, $version, $operator);
	}
	
	/**
	 * Restore Joomla's DB connection
	 * 
	 * @access protected
	 * @since 1.0
	 */
	function restoreJoomlaDb()
	{
		if ($this->compareJoomlaVersion('1.5.0'))
		{
			...
			if (phpversion() < '4.2.0')
			{
				if (!($db->_resource = @mysql_connect( $host, $user, $password )))
				{
					$this->raiseError('Could not connect to MySQL');
				}
			}
			else
			{
				// true forces a new connection even if the same username and password
			...
			if ($database != '' && !mysql_select_db( $database, $db->_resource ))
			{
			...
			if ( strpos( $db->getVersion(), '5' ) === 0 )
			{
				$db->setQuery( "SET sql_mode = 'MYSQL40'" );
				$db->query();
			}
		}
		else
	...
	function _parseConfiguration()
	{
		...
		// Always restore DB (though it seems to be only necessary when CakePHP uses a different DB than Joomla, but just to be sure let's do it)
		
		$this->restoreJoomlaDb = true;
	}
